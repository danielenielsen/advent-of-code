...
fn handleLine(linedata: *const LineData) i32 {
    const ribbon_wrap: i32 = @min(
        2 * linedata.length + 2 * linedata.width,
        2 * linedata.length + 2 * linedata.height,
        2 * linedata.width + 2 * linedata.height,
    );
    const ribbon_bow: i32 = linedata.length * linedata.width * linedata.height;
