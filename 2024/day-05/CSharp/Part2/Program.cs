namespace Part2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Update> notValidUpdates = updates.Where(x => !x.IsValid(rules)).ToList();
            notValidUpdates.ForEach(x => x.Sort(rules));

            int result = notValidUpdates.Select(x => x.GetMiddleNumber()).Sum();
