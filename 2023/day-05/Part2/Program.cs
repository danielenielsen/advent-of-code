...
namespace Part2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            List<(long, long)> seedPairs = seedInformation.Seeds.Where((val, index) => index % 2 == 0).Select((val, index) => (val, seedInformation.Seeds[index * 2 + 1])).ToList();
            long lowestDistance = long.MaxValue;
            foreach (var pair in seedPairs)
            {
                long rangeStart = pair.Item1;
                long rangeLength = pair.Item2;

                for (long i = 0; i < rangeLength; i++)
                {
                    long seed = rangeStart + i;

                    long tmp = seedToSoilMapping(seed);
                    tmp = soilToFertilizerMapping(tmp);
                    tmp = fertilizerToWaterMapping(tmp);
                    tmp = waterToLightMapping(tmp);
                    tmp = lightToTemperatureMapping(tmp);
                    tmp = temperatureToHumidityMapping(tmp);
                    tmp = humidityToLocationMapping(tmp);
