...
internal static class Program
{
    ...
    private static long Solve(List<Coordinates> coordinates)
    {
        return Time("Solve body", () =>
        {
            ...
            List<List<Coordinates>> circuits = coordinates.Select(x => new List<Coordinates> { x }).ToList();

            foreach ((Coordinates first, Coordinates second, _) in combinations.OrderBy(x => x.distance))
            {
                int? res1 = null;
                int? res2 = null;
            ...
            throw new Exception("Expected to return when all circuits are combined in the loop.");
        });
    }
